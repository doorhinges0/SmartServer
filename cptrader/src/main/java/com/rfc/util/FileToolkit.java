...
//			log.warn("要删除得目录不存在! path = " + paramString);
	...
	public static void append2File(String filePathAndName, String content){
		...
		try {
			// 打开一个随机访问文件流，按读写方式
			randomFile = new RandomAccessFile(filePathAndName, "rw");
			// 文件长度，字节数
	...
	public static void append2File2(String filePathAndName, String content){
		content = content + System.getProperty("line.separator");
		RandomAccessFile randomFile = null;
		try {
			// 打开一个随机访问文件流，按读写方式
	...
	public static String getPath(){
		...
		System.out.println("=====filePath======"+filePath); //输出内容
		System.out.println("=====pathSplit======"+pathSplit); //输出内容
...
//          System.out.println("getPath2:"+filePath.lastIndexOf("/"));
	...
	public static String getProjectPath() {
		...
		URL url = FileToolkit.class .getProtectionDomain().getCodeSource().getLocation();
	...
	public static String getProjectPath4Log() {
		...
		if (filePath.endsWith(".jar"))
			filePath = filePath.substring(0, filePath.lastIndexOf("/") + 1);
//		System.out.println("getProjectPath4Log :filePath"+filePath);
		filePath = filePath.substring(0, filePath.lastIndexOf("/") );
//		System.out.println("getProjectPath4Log :filePath"+filePath);
		filePath = filePath.substring(0, filePath.lastIndexOf("/") );
//		System.out.println("getProjectPath4Log :filePath"+filePath);
		File file = new File(filePath);
		filePath = file.getAbsolutePath() + File.separator + "logs" + File.separator;
		return filePath;
	}
	...
	public static String getProjectPath2() {
		...
		try {
			filePath = URLDecoder.decode (url.getPath(), "utf-8");
			System.out.println("=====URL path======"+filePath); //输出内容
		} catch (Exception e) {
			e.printStackTrace();
		}
		return  filePath;
